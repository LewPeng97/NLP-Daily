...
def print_basic_info(modules, consts, options):
    ...
    if options["has_learnable_w2v"]:# 使用已经训练好的word2vec词向量
...
def init_modules():
    ...
    options["omit_eos"] = True # omit <eos> and continuously decode until length of sentence reaches MAX_LEN_PREDICT (for DUC testing data)
...
def run(existing_model_name = None):
    modules, consts, options = init_modules() #词表参数，常参，系统参数

    if options["is_predicting"]: #预测阶段
    ...
    print_basic_info(modules, consts, options) #打印参数
    ...
    if True: #TODO: refactor
        ...
        model_name = "".join(["lcsts.s2s.", options["cell"]])
        existing_epoch = 0
        if need_load_model:
            if existing_model_name == None:
                existing_model_name = "lcsts.s2s.transformer.gpu1.epoch27.2"
            ...
            model, optimizer = load_model(existing_model_name, model, optimizer)
