...
"""
读取原文内容，保存格式为(lines,f_cont),并截断超过MAX_LEN_X的原文内容; lines-->['新华社'，‘可以’]; f_cont-->'新华社 可以'
"""
def read_cont(f_cont, cfg):
    ...
    return (lines, f_cont) if len(lines) <= cfg.MAX_LEN_X+1 else None
"""
将目标摘要“<s>摘要内容</s>”转化为“摘要内容”
"""
...
"""
读取目标摘要内容，保存格式为(lines,sent),并截断超过MAX_LEN_Y的目标摘要内容
"""
def read_head(f_head, cfg):
    ...
    return (lines, sents) if len(lines) <= cfg.MAX_LEN_X+1 else None
