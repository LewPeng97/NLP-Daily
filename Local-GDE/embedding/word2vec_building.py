...
if __name__=='__main__':
    ...
    train_words, test_words = parse_data('./word_data/train_big.csv',
                                         './word_data/eval.csv')
    logging.info("reading end !!!")
    logging.info("save cut words result !!!")
    save_data(train_words, test_words, './word_data/train.txt',
          './word_data/test.txt',
        '../stopwords/cn_stopwords.txt')
    logging.info("cut over !!!")
    logging.info("union all data to train word2vec !!!")
    save_sentences('./word_data/train.txt', './word_data/test.txt',
               './word_data/train_union_test.txt')

    logging.info("union end !!!")
    logging.info("create vocab dict !!!")
    lines = read_file('./word_data/train_union_test.txt')
    vocab, reverse_vocab = build_vocab(lines)

    save_word_dict(vocab, './dict_from_lcsts.txt')
    logging.info("saved vocab dict !!!")
    logging.info("begin to train LCSTS_W2V embedding !!!")
    build_w2v('./word_data/train_union_test.txt',
          './LCSTS_Words_Vectors.txt')
    logging.info("end to train LCSTS_W2V embedding !!!")
    end = datetime.datetime.now()
    logging.info("Word2Vec training time: {}".format(end - start))

    model = Word2Vec.load('./w2v.model')
    print(model.most_similar(u'新闻',topn=10))

    word_2x = np.load('./w2v.model.trainables.syn1neg.npy')
